function loadClassifyHtml() {
    return {
        setClassifySelect: function() {
            return {
                set: function() {
                    ...
                    CLASSIFYS.forEach(r => {
                        classifyListHtml += `
                            <span class="" id="i_mitem_classify_select_el_box_${r.classifyUuid}">
                                <button id="i_ty_mitem_classify_select_el_btn_${r.classifyUuid}" class="btn ty-st-mItem-item-btn" onclick="eventClassifyHandler().classifyInfoOpenClick('${r.classifyUuid}')">${r.classifyName}</button>
                            </span>
                        `;
                    })
                    let html = `
        ...
        setClassifyInfo: function() {
                // $(`#i_ty_mitem_classify_select_el_btn_${SELECTED_CLASSIFY.classifyUuid}`).addClass('ty-st-sd-batch-selected-item-btn-active')
                let html = `
