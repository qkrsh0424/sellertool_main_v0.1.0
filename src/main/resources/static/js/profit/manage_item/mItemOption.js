        ...
        setOptionSelect: function () {
            return {
                set: function () {
                    ...
                    OPTIONS.forEach(r => {
                        optionListHtml += `
                            <span class="st-sd-batch-selected-item-wrapper" id="i_mitem_option_select_el_box_${r.optionUuid}">
                                <button class="btn" onclick="eventOptionHandler().optionInfoOpenClick('${r.optionUuid}')">${r.optionName}</button>
                            </span>
                        `;
                    })
                    let html = `
...
function eventOptionHandler() {
    return {
        ...
        optionInfoOpenClick: async function(optionUuid){
            loadOptionHtml().setOptionSelect().set();
            $(`#i_mitem_option_select_el_box_${optionUuid}`).css('border','2px solid #f2a057');
